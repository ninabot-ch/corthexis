...
Reads every note (``$HEXIS_NOTES_DIR/*.md``, excluding the generated
``MEMORY.md`` index), splits each into chunks, embeds them (see
``hexis.embeddings`` — multilingual by default, so notes written in one
language are recalled by queries in another) and stores unit-normalized
vectors in a local SQLite database. The MCP server (``hexis.server``) then
...
files disappeared are pruned. A few hundred notes index in seconds.
    ...
    python -m hexis.index            # incremental
    python -m hexis.index --rebuild  # drop and rebuild every vector
...
MEMORY_DIR = Path(os.environ.get("HEXIS_NOTES_DIR", "./notes")).expanduser()
DB_PATH = Path(
...
def embed_batch(texts: list[str]) -> list[list[float]]:
    """Embed via the configured backend (see ``HEXIS_EMBED_BACKEND``)."""
    return embed(texts)
...
def write_memory_index(con: sqlite3.Connection) -> None:
    ...
    header = (
        "<!-- Generated by `python -m hexis.index` from the notes' frontmatter "
